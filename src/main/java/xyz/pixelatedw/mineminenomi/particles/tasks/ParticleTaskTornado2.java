...
public class ParticleTaskTornado2 extends TimerTask
{
	...
	public static ParticleTaskTornado2 Create(LivingEntity player, double posX, double posY, double posZ, Object particle, double radius, int density)
	{
		return new ParticleTaskTornado2(player, posX, posY, posZ, particle, radius, density);
	}
	
	private ParticleTaskTornado2(LivingEntity player, double posX, double posY, double posZ, Object particle, double radius, int density)
	{
		this.player = player;
				...
				for(double i = 0; i <= 1; i += 1)
				{
					...
					try
					{
						if(this.particle instanceof CustomParticle)
						{
							...
							player.world.addParticle((IParticleData) particle, this.posX + x, this.posY - 0.7 + y, this.posZ + z, 0.0D, 0.0D, 0.0D);
						Thread.sleep(1);
					} 

...
public class ParticleTaskGeyserExplosion extends TimerTask
{
	...
	public ParticleTaskGeyserExplosion(LivingEntity player, double posX, double posY, double posZ, String fxName)
	{
		this.player = player;
		this.fxName = fxName;
		this.posX = posX;
		...
		while(t < 10)
		{
			...
			for(double theta = 0; theta <= 2 * Math.PI; theta = theta + Math.PI / 32)
			{
				...
				try
				{
					...
					//	ModMain.proxy.spawnCustomParticles(this.player, this.fxName.replace("custom_", ""), this.posX + x, this.posY + y, this.posZ + z, 0.0D, 0.0D, 0.0D);
					//else							

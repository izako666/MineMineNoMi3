...
public class CommandDoriki
{
	public static void register(CommandDispatcher<CommandSource> dispatcher)
	{
		LiteralArgumentBuilder<CommandSource> builder = Commands.literal("doriki").requires(source -> source.hasPermissionLevel(2));

		builder
			.then(Commands.literal("+")
				.then(Commands.argument("value", IntegerArgumentType.integer(1, Values.MAX_DORIKI))
			...
			.then(Commands.literal("=")
				...
				.then(Commands.literal("MAX")
					.then(Commands.argument("targets", EntityArgument.players())
						.executes(context ->
							{
								return setValue(context, Values.MAX_DORIKI, EntityArgument.getPlayers(context, "targets")); 
							}
						)
					)
				)

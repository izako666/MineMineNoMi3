...
public class RenderZoanMorph extends Render
{
	...
	public RenderZoanMorph(RenderManager renderManager, EntityModel model, String texture, double scale, float[] offset)
	{
		...
		/*if (texture.contentEquals("$playerskin"))
		{
			...
			ResourceLocation rs = DefaultPlayerSkin.getDefaultSkin(minecraft.player.getUniqueID());
	...
	public void doRender(Entity entity, double x, double y, double z, float u, float v)
	{
		...
		if(entityLiving != Minecraft.getInstance().player)
		...
		GL11.glPushMatrix();
		{
			...
			RendererModel hand = ((ModelZoanMorph) this.model).getHandRenderer();
			if(hand != null)
			{
	...
	protected void renderEquippedItems(LivingEntity entity, float age)
	{
		...
		if (itemstack != null && itemstack.getItem() != null)
		{
			...
			Optional<Object[]> opt = null;/*Arrays.stream(MorphsHelper.getMorphsMap().get(props.getDevilFruit())).filter(x ->
			{
